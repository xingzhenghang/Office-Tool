...
Office Tool Plus 需要 Net Framework 4.6.1 的支持，如 Windows 7 系统无法正常启动 Office Tool Plus，请尝试安装对应版本的 Net Framework，[点击此处下载](http://go.microsoft.com/fwlink/?linkid=780597) 在线安装包或者 [点击此处下载](http://go.microsoft.com/fwlink/?LinkId=780601) 离线安装包
...
## Office Tool Plus 详细的介绍

### 管理 Office

Office Tool Plus 仅支持管理 Click To Run 版本的 Office，其他版本的 Office 不受支持。如果您还在使用旧版本的 Office，我们建议您卸载旧版本的 Office 并使用 Office Tool Plus 安装新版 Office。
...
ClickToRun 是我们平常看到的检查更新以及卸载 Office 的程序。更改 Click to Run 的语言后，必须重新启动 ClickToRun 服务才能使更改生效。若您不会重启服务，您可以选择重启计算机。
...
点击查询版本号可以从微软服务器获取各个通道的 Office 的版本号以及发布日期。随后可以在版本号下拉框中选择 Office 的版本，如不选择，默认下载/安装最新版本。
...
若要分享安装文件，您可以选择将下载好的 Office 文件夹分享给其他人，或者是使用 Office Tool Plus 创建 ISO 文件并与他人共享。
当您使用 Office Tool Plus 创建 ISO 文件时，请务必确保 Office 文件夹与 Office Tool Plus 同目录（默认同目录），否则创建的 ISO 文件将不会包含离线文件。
创建的 ISO 文件可以直接挂载并使用，其内置 Office Tool Plus，可以直接运行。

#### 定位安装文件

若要选择安装文件，请定位一个安装文件，Office Tool Plus 即可自动识别安装文件。
如果是 ISO 镜像文件，请先挂载镜像文件，再使用 Office Tool Plus 定位安装文件。PS：我们不推荐使用 ISO 镜像搭配 Office Tool Plus 进行安装；直接在程序中下载安装文件和下载 ISO 文件是一样的安装包，而且程序下载的始终都是最新版本。
...
在语言列表中，Full 代表这个语言在 Office 内可以以所选语言显示界面、帮助以及拥有所选语言的校对工具。

Partial 代表这个语言包含某些 Office 应用程序的所选界面语言，同时也包含校对工具。

Proofing 代表这个语言在 Office 内只包含拼写检查器。

注意：在安装完整的 Office 时，Full 类型的语言是不可缺少的，比如你需要安装一个 Partial 或者 Proofing 类型的语言时，必须确保需要选择一个 Full 类型的语言，否则 Office 将无法正常安装。如果您没有下载一个 Full 类型的语言包，则您只可以安装语言包或者语言校对工具而无法安装完整的 Office。
...
迅雷支持 IE/HTTP/SOCK/FTP 代理，选择代理类型，输入代理的信息，点击应用即可。注意：您只能在开始下载前进行设置，下载过程中无法更改代理设置。如果中途需要更改代理设置，可以取消下载再更改，迅雷支持断点续传。

同时迅雷还支持设置下载限速，单位为 KB/s，如果需要取消限制，输入 0 即可。

Aria2c 支持设置 HTTP/FTP/SFTP 代理，当代理类型选择 HTTP/FTP/SFTP 时，只需要填写相关信息即可成功设置代理。注意：您只能在开始下载前进行代理设置。
...
引擎|说明
:---|:---
迅雷下载|显示下载进度、文件大小、下载速度以及文件名等信息，支持断点续传，支持设置限速
Aria2c|在一个单独的 CMD 窗口中显示下载信息（请勿点击 CMD 窗口避免下载被暂停）
微软官方|仅有下载状态显示，没有速度，没有文件信息
...
Click to Run 默认将 Office 安装至系统盘，此设置由微软管理，无法更改。注意：我们不推荐使用修改注册表的方式更改 Office 的安装位置，因为这可能会引发一系列的问题。

#### 安装语言设置说明

主要语言将会被用于确定 Shell UI 区域性，包括快捷方式，右键单击上下文菜单和工具提示。
...
注意：在安装完整的 Office 时，Full 类型的语言是不可缺少的，比如你需要安装一个 Partial 或者 Proofing 类型的语言时，必须确保需要选择一个 Full 类型的语言，否则 Office 将无法正常安装。如果您没有选择一个 Full 类型的语言包，则您只可以安装语言包或者语言校对工具而无法安装完整的 Office。

安装语言默认匹配系统语言，但是这会导致同时安装中文版以及英文版的 Office。如果不想安装多语言的 Office，请自行从列表中选择语言。
...
有关产品 ID 的信息，请前往[微软的网站](https://go.microsoft.com/fwlink/p/?LinkID=301891)查看详细信息

#### 常见问题以及解决办法（这些问题大部分针对的是国内用户，因此这段文本不需要翻译，直接删除即可）
...
解决办法2|尝试使用 Office Tool Plus Lite 安装 Office。
...
举个例子：比如要转换 Office 2019 Retail -> Office 2019 Volume，安装 Office 2019 Volume 许可证即可完成转换。
安装完成后，可以使用对应的密钥来激活，同时也可以做到多授权版本共存。

点击旁边的省略按钮可以自定义选择并安装证书。

只有当计算机上安装了基于 C2R 版本的 Office 时，Office Tool Plus 才会读取 Office 证书并在下拉框中显示。否则 Office Tool Plus 仅会显示内置的 Volume (批量版) 证书，Office Tool Plus 则什么都不会显示。
...
*注意：如果 KMS 服务器的管理员修改了服务端的 KMS 配置，则您的续期时间可能不是 7 天，具体可以咨询 KMS 服务器管理员以获取其他信息。*

Office Tool Plus 保证不会对用户的计算机进行任何未经授权的修改，更不会进行任何破解。
...
#### 常见错误以及解决办法（这些问题有针对性，各个语言之间的用户可能并不通用）
